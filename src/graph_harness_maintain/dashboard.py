...
function setGraphScope() {{ activeProfile=document.getElementById('profile-select').value; activeProject=document.getElementById('project-select').value; selectedGraphRef=null; if(activeProfile==='ehrlab') {{ document.getElementById('mode-help').textContent='No projects archived for this profile yet'; }} else {{ document.getElementById('mode-help').textContent='Overview = curated system map. Focus = selected item plus 1-hop neighbors. Full graph = debug/all nodes.'; }} renderGraph(); renderGraphSummary(true); updateViewInLogsControl(); }}
