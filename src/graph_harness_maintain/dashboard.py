...
.shell {{ display:grid; grid-template-columns:86px 1fr; height:calc(100vh - 64px); min-height:0; }}
...
.graph-workspace {{ height:calc(100% - 76px); min-height:0; display:grid; grid-template-columns:minmax(0,1fr) 300px; border:1px solid var(--line); border-radius:18px; overflow:hidden; background:#fff; box-shadow:0 16px 40px rgba(16,24,40,.06); }}
.graph-main {{ display:grid; grid-template-rows:auto 1fr auto; min-width:0; min-height:0; }} .toolbar {{ display:flex; align-items:center; justify-content:space-between; gap:10px; padding:10px 12px; border-bottom:1px solid var(--line); }} .toolbar-left,.legend {{ display:flex; align-items:center; gap:7px; flex-wrap:wrap; }}
...
.type-filter-panel {{ display:flex; align-items:center; gap:5px; flex-wrap:wrap; padding:0; border-bottom:0; background:transparent; max-width:48vw; }} .filter-chip {{ border:1px solid var(--line); background:#fff; border-radius:999px; padding:4px 8px; font-size:11px; cursor:pointer; }} .filter-chip.active {{ background:var(--accent-soft); color:var(--accent); border-color:var(--accent); }} .count-pill {{ color:var(--muted); font-size:12px; margin-left:6px; white-space:nowrap; }}
...
.logs-layout {{ display:grid; grid-template-columns:280px minmax(380px,1fr) 440px; height:calc(100% - 70px); min-height:0; border:1px solid var(--line); border-radius:18px; background:#fff; overflow:hidden; box-shadow:0 16px 40px rgba(16,24,40,.06); }}
.explorer,.preview-panel {{ min-width:0; min-height:0; overflow:auto; }} .file-table {{ min-width:0; min-height:0; overflow:hidden; border-right:1px solid var(--line); display:grid; grid-template-rows:auto minmax(0,1fr); }} .explorer {{ border-right:1px solid var(--line); padding:16px; }} .pane-head {{ display:flex; justify-content:space-between; align-items:center; gap:8px; padding:15px 16px; border-bottom:1px solid var(--line); }} .file-table-scroll {{ overflow-y:scroll; overflow-x:auto; min-height:0; height:100%; overscroll-behavior:contain; scrollbar-gutter:stable; border-top:1px solid var(--line); }} .file-table-scroll thead th {{ position:sticky; top:0; z-index:2; box-shadow:0 1px 0 var(--line); }} .file-table-scroll table {{ min-width:760px; }} .file-table-scroll::-webkit-scrollbar, .explorer::-webkit-scrollbar, .preview-panel::-webkit-scrollbar, pre::-webkit-scrollbar {{ width:10px; height:10px; }} .file-table-scroll::-webkit-scrollbar-thumb, .explorer::-webkit-scrollbar-thumb, .preview-panel::-webkit-scrollbar-thumb, pre::-webkit-scrollbar-thumb {{ background:#cbd5e1; border-radius:999px; }}
...
function startCanvasPan(ev) {{ if (!(ev.button === 0 || ev.button === 2)) return; if (ev.target.closest && ev.target.closest('.node-card')) return; ev.preventDefault(); isCanvasPanning = true; canvasPanStart = {{x:ev.clientX, y:ev.clientY, px:panZoomState.x, py:panZoomState.y}}; document.getElementById('graph-canvas').classList.add('panning'); }}
