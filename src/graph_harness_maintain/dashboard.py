...
.graph-main {{ display:grid; grid-template-rows:auto 1fr auto; min-width:0; }} .toolbar {{ display:flex; align-items:center; justify-content:space-between; gap:10px; padding:14px; border-bottom:1px solid var(--line); }} .toolbar-left,.legend {{ display:flex; align-items:center; gap:8px; flex-wrap:wrap; }}
...
.explorer,.file-table,.preview-panel {{ min-width:0; overflow:auto; }} .explorer {{ border-right:1px solid var(--line); padding:16px; }} .file-table {{ border-right:1px solid var(--line); }} .pane-head {{ display:flex; justify-content:space-between; align-items:center; gap:8px; padding:15px 16px; border-bottom:1px solid var(--line); }}
.tree {{ list-style:none; padding-left:0; margin:10px 0; }} .tree ul {{ list-style:none; padding-left:18px; margin:4px 0; }} .tree li {{ padding:5px 7px; border-radius:9px; color:#344054; }} .tree li.active {{ background:var(--accent-soft); color:var(--accent); font-weight:700; }}
...
<body>
<header class="topbar"><div class="brand"><div class="logo">◆</div><span>Governance Hub</span><span class="badge">v2.0</span></div><div class="top-actions"><a class="btn" href="#/logs" id="top-logs-link">Logs</a><button class="btn" aria-label="Theme toggle visual only">☼</button><div class="avatar">A</div><span>Admin</span></div></header>
...
function initPositions() {{ const cols = 5; DATA.graph.nodes.forEach((n,i) => positions[n.id] = positions[n.id] || {{x:90+(i%cols)*190, y:70+Math.floor(i/cols)*105}}); }}
...
function renderLegend() {{ const types = ['tool','knowledge_source','session','policy','artifact','proposal','provenance_state']; document.getElementById('graph-legend').innerHTML = types.map(t => `<span class="legend-item"><i class="dot" style="background:${{colorMap[t][1]}}"></i>${{t.replace('_',' ')}}</span>`).join(''); }}
function renderGraph() {{ initPositions(); const edgeLayer = document.getElementById('edge-layer'); const nodeLayer = document.getElementById('node-layer'); edgeLayer.innerHTML=''; nodeLayer.innerHTML=''; DATA.graph.edges.forEach(e => {{ const s=positions[e.source], t=positions[e.target]; if(!s||!t) return; const midx=(s.x+t.x)/2, midy=(s.y+t.y)/2-18; const path=document.createElementNS('http://www.w3.org/2000/svg','path'); path.setAttribute('class','edge-line'); path.setAttribute('d',`M ${{s.x+70}} ${{s.y+20}} Q ${{midx}} ${{midy}} ${{t.x+70}} ${{t.y+20}}`); path.setAttribute('marker-end','url(#arrow)'); path.addEventListener('click', ev => {{ ev.stopPropagation(); selectEdge(e); }}); edgeLayer.appendChild(path); const label=document.createElementNS('http://www.w3.org/2000/svg','text'); label.setAttribute('class','edge-label'); label.setAttribute('x',midx); label.setAttribute('y',midy); label.textContent=e.relation || e.type; edgeLayer.appendChild(label); }}); DATA.graph.nodes.forEach(n => {{ const p=positions[n.id]; const colors=colorMap[n.type] || colorMap.requirement; const g=document.createElementNS('http://www.w3.org/2000/svg','g'); g.setAttribute('class','node-card'); g.setAttribute('transform',`translate(${{p.x}} ${{p.y}})`); g.dataset.id=n.id; g.innerHTML=`<rect width="150" height="44" fill="${{colors[0]}}" stroke="${{colors[1]}}"></rect><text x="12" y="18">${{escapeSvg(n.label || n.id).slice(0,22)}}</text><text x="12" y="34" fill="#667085">${{escapeSvg(n.type)}}</text>`; g.addEventListener('mousedown', ev => startNodeDrag(ev,n.id)); g.addEventListener('click', ev => {{ ev.stopPropagation(); selectNode(n); }}); nodeLayer.appendChild(g); }}); applyZoom(); renderMinimap(); }}
...
function selectEdge(e) {{ document.getElementById('node-inspector').textContent=e.label || e.type; document.getElementById('inspector-body').innerHTML = kv({{ID:e.id, Type:'edge', Relation:e.relation || e.type, Source:e.source, Target:e.target, Status:e.status || 'active'}}); }}
...
function renderMinimap() {{ const mini=document.getElementById('minimap'); mini.innerHTML=''; Object.entries(positions).slice(0,40).forEach(([id,p]) => {{ const node=DATA.graph.nodes.find(n=>n.id===id)||{{type:'requirement'}}; const d=document.createElement('i'); d.className='mini-node'; d.style.left=(8+(p.x%120))+'px'; d.style.top=(10+(p.y%60))+'px'; d.style.background=(colorMap[node.type]||colorMap.requirement)[1]; mini.appendChild(d); }}); }}
function renderTree() {{ const groups=['artifacts','sessions','proposals','policies','provenance','system']; const inv=DATA.file_inventory; document.getElementById('explorer-count').textContent=inv.length+' items'; document.getElementById('file-tree').innerHTML=groups.map(g=>`<li class="${{g==='artifacts'?'active':''}}">▾ 📁 ${{g}}/ <span class="badge">${{inv.filter(i=>i.group===g || (i.groups||[]).includes(g)).length}}</span><ul>${{inv.filter(i=>i.group===g || (i.groups||[]).includes(g)).slice(0,8).map(i=>`<li>◇ ${{i.name}}</li>`).join('')}}</ul></li>`).join(''); }}
function renderFiles() {{ const q=(document.getElementById('file-search').value||'').toLowerCase(); const rows=DATA.file_inventory.filter(i=>!q || i.path.toLowerCase().includes(q)); document.getElementById('table-count').textContent=rows.length+' items'; document.getElementById('file-rows').innerHTML=rows.map((f,i)=>`<tr class="${{i===0?'selected':''}} level-${{f.level}}" data-path="${{f.path}}"><td>▧ ${{f.name}}</td><td>${{f.type}}</td><td>${{f.size_label}}</td><td>${{f.modified}}</td><td><span class="level-dot"></span>${{f.level}}</td></tr>`).join(''); document.querySelectorAll('#file-rows tr').forEach(row => row.onclick=()=>selectFile(row.dataset.path)); if(rows[0] && !selectedFile) selectFile(rows[0].path); }}
...
function showPreviewTab(tab) {{ if(!selectedFile) return; document.querySelectorAll('[data-preview-tab]').forEach(b=>b.classList.toggle('active', b.dataset.previewTab===tab)); let text=selectedFile.preview; if(tab==='metadata') text=JSON.stringify({{path:selectedFile.path,type:selectedFile.type,size:selectedFile.size,modified:selectedFile.modified,source:selectedFile.source,level:selectedFile.level}}, null, 2); if(tab==='lineage') text=JSON.stringify(selectedFile.lineage, null, 2); document.getElementById('preview-code').textContent=text; }}
...
renderLegend(); renderGraph(); renderTree(); renderFiles(); selectNode(DATA.graph.nodes[0] || {{id:'none',type:'unknown',label:'No graph nodes'}});
</script>
</body>
</html>
"""
