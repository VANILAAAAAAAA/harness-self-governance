...
function updateViewInLogsControl() {{ const link=document.getElementById('view-in-logs'); const note=document.getElementById('log-mapping-note'); const path=selectedGraphRef && selectedGraphRef.path; const can=!!(path && DATA.file_inventory.some(f=>f.path===path)); link.classList.toggle('disabled', !can); link.setAttribute('aria-disabled', String(!can)); link.textContent = can ? 'View exact artifact in Logs →' : 'No direct log mapping'; link.title = can ? path : 'No matching local log/artifact is indexed for this graph item.'; if(note) note.textContent = can ? `Exact mapping: ${{path}}` : 'No exact indexed local artifact path for this graph item yet; related-log mapping is deferred.'; }}
...
function renderNodeInspector(n) {{ document.getElementById('node-inspector').textContent=n.label || n.id; const incidentEdges=DATA.graph.edges.filter(e=>e.source===n.id || e.target===n.id); document.getElementById('inspector-body').innerHTML = kv({{Mode:'Node', ID:n.id, Type:n.type, Status:n.status, IncidentEdges:incidentEdges.length, Focus:'Use Focus · 1-hop to show this node and immediate neighbors.', Description:n.description || n.summary || '', Tags:(n.tags||[]).join(', '), Path:(n.path || (n.metadata||{{}}).path || '')}}); updateViewInLogsControl(); }}
function renderEdgeInspector() {{ const e=selectedEdge(); if(!e) {{ document.getElementById('node-inspector').textContent='Edge details'; document.getElementById('inspector-body').innerHTML='<span class="subtitle">Select an edge in the graph to inspect source, target, relation, and metadata.</span>'; updateViewInLogsControl(); return; }} const source=DATA.graph.nodes.find(n=>n.id===e.source)||{{}}; const target=DATA.graph.nodes.find(n=>n.id===e.target)||{{}}; document.getElementById('node-inspector').textContent=e.label || e.type || 'Edge'; document.getElementById('inspector-body').innerHTML = kv({{Mode:'Edge', ID:selectedGraphRef.id, Relation:e.relation || e.type, Source:e.source, Target:e.target, SourceType:source.type || 'unknown', TargetType:target.type || 'unknown', Confidence:e.confidence, LogPath:selectedGraphRef.path, Status:e.status || 'active', Metadata: JSON.stringify(e.metadata || {{}})}}); updateViewInLogsControl(); }}
