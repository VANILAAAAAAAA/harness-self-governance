...
function visibleNodes() {{ const q=(document.getElementById('graph-search').value||'').toLowerCase(); return DATA.graph.nodes.filter(n => scopeAllowsNode(n) && modeNodeAllowed(n) && activeGraphTypes.has(n.type) && !excludedGraphTypes.has(n.type) && (!q || nodeSearchText(n).includes(q))); }}
