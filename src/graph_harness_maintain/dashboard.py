...
def _html(data: dict[str, Any]) -> str:
    ...
    project_options = "".join(
        f'<option value="{manifest.get("project_id")}">{manifest.get("project_id")}</option>'
...
.logic-flow {{ display:flex; align-items:center; gap:8px; margin:-2px 0 10px; padding:7px 10px; border:1px solid var(--line); border-radius:14px; background:#fbfcff; color:#475467; font-size:12px; overflow:hidden; white-space:nowrap; }} .logic-flow strong {{ color:var(--ink); }} .logic-flow .arrow {{ color:var(--muted); }}
.graph-workspace {{ height:calc(100% - 112px); min-height:0; display:grid; grid-template-columns:minmax(0,1fr) 300px; border:1px solid var(--line); border-radius:18px; overflow:hidden; background:#fff; box-shadow:0 16px 40px rgba(16,24,40,.06); }}
...
function fitGraphToVisible() {{
    ...
    const bounds = nodeList.reduce((acc,n)=>{{ const p=positions[n.id] || {{x:0,y:0}}; acc.minX=Math.min(acc.minX,p.x); acc.minY=Math.min(acc.minY,p.y); acc.maxX=Math.max(acc.maxX,p.x+150); acc.maxY=Math.max(acc.maxY,p.y+44); return acc; }}, {{minX:Infinity,minY:Infinity,maxX:-Infinity,maxY:-Infinity}});
    const padding = 54;
...
function overviewNodeIds() {{ const priorityTypes=new Set(['profile','project','project_summary','session','knowledge_source','tool','proposal','policy','gate','artifact','report','provenance_state','pipeline_run','decision','requirement','constraint']); const priorityWords=/profile|project|session|dashboard|pipeline|policy|gate|proposal|provenance|artifact|report|tool|storage-guard|safety|approval|governance/i; const chosen=[]; const add=n=>{{ if(n && !chosen.includes(n.id)) chosen.push(n.id); }}; DATA.graph.nodes.filter(n=>['profile','project','project_summary'].includes(n.type)).forEach(add); (DATA.graph_summary.hubs||[]).slice(0,6).forEach(h=>add(DATA.graph.nodes.find(n=>n.id===h.id))); DATA.graph.nodes.filter(n=>priorityTypes.has(n.type) && priorityWords.test(`${{n.id}} ${{n.label}} ${{n.path||''}}`)).forEach(add); DATA.graph.nodes.filter(n=>['tool','policy','gate','proposal','pipeline_run','provenance_state'].includes(n.type)).forEach(add); return new Set(chosen.slice(0,28)); }}
...
function visibleNodes() {{ const q=(document.getElementById('graph-search').value||'').toLowerCase(); return DATA.graph.nodes.filter(n => scopeAllowsNode(n) && modeNodeAllowed(n) && activeGraphTypes.has(n.type) && (!q || nodeSearchText(n).includes(q))); }}
...
function clearGraphFilters() {{ activeGraphTypes = new Set(DATA.graph_filter_types || []); activeEdgeTypes = new Set(DATA.edge_filter_types || []); document.getElementById('graph-search').value=''; renderTypeFilters(); renderEdgeFilters(); renderGraph(true); }}
...
function setGraphScope() {{ activeProfile=document.getElementById('profile-select').value; activeProject=document.getElementById('project-select').value; selectedGraphRef=null; if(activeProfile==='ehrlab') {{ document.getElementById('mode-help').textContent='No projects archived for this profile yet'; }} else {{ document.getElementById('mode-help').textContent='Overview = curated system map. Focus = selected item plus 1-hop neighbors. Full graph = debug/all nodes.'; }} renderGraph(true); renderGraphSummary(true); updateViewInLogsControl(); }}
document.getElementById('profile-select').addEventListener('change', setGraphScope); document.getElementById('project-select').addEventListener('change', setGraphScope);
